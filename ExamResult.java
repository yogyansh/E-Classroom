...
public class ExamResult extends JFrame implements ActionListener {
    ...
    public ExamResult() {
        ...
        try {
            ...
            ResultSet rs = connect.statement.executeQuery("Select count(StudentId)from examAns");
            rs.next();
            int count = rs.getInt(1);
            String data[][] = new String[count][4];
            String ColName[] = {"Question ID", "Answer", "Marks", "Student ID"};
            rs = connect.statement.executeQuery("Select * from examAns");
